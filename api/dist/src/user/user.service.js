...
const role_entity_1 = require("../auth/entites/role.entity");
const typeorm_2 = require("typeorm");
const config_1 = require("../config/config");
const mail_service_1 = require("./mail.service");
let UserService = class UserService {
    constructor(userRepository, roleRepository, mailService, logger) {
    ...
    async requestDeactivation(userId) {
        const user = await this.userRepository.findOne({
            where: { id: userId },
            relations: ['role'],
        });
        ...
        const deactivationLink = `${config_1.config.get().APP_URL}/user/deactivate/${token}`;
    ...
    async removeByToken(token) {
        ...
        if (!user) {
            throw new common_1.NotFoundException('Invalid or expired deactivation token');
        }
        return this.userRepository.remove(user);
    }
};
