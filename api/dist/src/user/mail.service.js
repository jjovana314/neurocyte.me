...
let MailService = class MailService {
    constructor() {
        this.transporter = nodemailer.createTransport({
            host: config_1.config.get().MAIL_HOST,
            port: Number(config_1.config.get().MAIL_PORT),
            auth: {
                user: config_1.config.get().MAIL_USER,
                pass: config_1.config.get().MAIL_PASS,
            },
        });
    }
    async sendDeactivationEmail(adminEmail, deactivationLink, userName) {
        await this.transporter.sendMail({
            from: config_1.config.get().MAIL_FROM,
            to: adminEmail,
