...
export async function addMigraineLog(
  patientId: number,
...
export async function addNcsStudy(
  patientId: number,
  dto: CreateNcsStudyDto,
): Promise<NcsStudy> {
  const { data } = await client.post<NcsStudy>(`/patients/${patientId}/ncs-studies`, dto);
  return data;
}
